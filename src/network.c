...
box **make_boxes_array(network *net, int batchNum)
{
    layer l = net->layers[net->n-1];
    box *boxes = calloc(l.w*l.h*l.n, sizeof(box));

    box **box_array = calloc(batchNum, sizeof(boxes));
    return box_array;
}

int *make_index_array(int batchNum)
{
    int *index_array = calloc(batchNum, sizeof(int));
    return index_array;
}

int *make_class_array(int batchNum)
{
    int *class_array = calloc(batchNum, sizeof(int));
    return class_array;
}

float ***make_probs_array(network *net, int batchNum)
{
    float ***prob_array = calloc(batchNum, sizeof(float**));
    return prob_array;
}
...
void network_detect_batch(network *net, matrix test, float thresh, float hier_thresh, float nms, box **boxes, float ***probs, int *idx, int* cls)
{
    float *out;
    int i,j,b;
	...
	float **outputLoc = calloc(net->batch, sizeof(float*));
	    ...
	    if (i+b < test.rows) {
                ...
                if (nms) 
                    do_nms_sort(boxesBatch, probsBatch, l.w*l.h*l.n, l.classes, nms);
